...
allowed: ${allowed.join(', ')}
  ...
  if (hasLocationAndJson) {
    throw new InvalidResponseError('`res` invoked with `location` and `json` keys: only one is allowed')
  }
  ...
  if (!hasOneOfLocationOrJson) {
    throw new InvalidResponseError('`res` must be invoked with either `location` or `json`')
  }
}
